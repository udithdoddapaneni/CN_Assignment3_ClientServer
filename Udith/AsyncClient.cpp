...
#define START                                                                  \
    0 // before the client starts up and sends the initial HELLO message
#define WAIT                                                                   \
    1 // while waiting for HELLO response from server. When successful moves to
      // READY state
#define READY                                                                  \
    2 // once it recieves HELLO it gets into READY state. Here it recieves ALIVE
      // from server for sending DATA. DATA is taken from stdin
#define READY_TIMER                                                            \
    3 // READY state takes input from STDIN and moves to READY_TIMER. When
      // READY_TIMER recieves ALIVE, it moves to READY STATE
#define CLOSING                                                                \
    4 // It gets into CLOSING state if it recieves eof or SIGINT or q or timeout
      // in the input from READY and WAIT states
#define CLOSED                                                                 \
    5 // it gets into CLOSED state from CLOSING on timeout or GOODBYE from
      // server

// NOTE: Client transitions to CLOSED state if it recieves GOODBYE no matter
...
void PayloadSend( int &sockfd, sockaddr *servaddr, socklen_t &socksize ) {
    ...
    if ( STATE == READY ) {
        ...
        if ( cin.eof() || ( input.length() == 1 && input[0] == 'q' ) ) {
            ...
            seq_no++;
            size_t sender_payload_size =
                min( RECV_BUFFER_SIZE - HEADER_SIZE, input.length() );
            ...
            MessageFactory::setSequenceNumber( seq_no, buffer );
            MessageFactory::setSessionId( session_id, buffer );
            MessageFactory::WriteMessage( buffer, payload, buffer,
                                          HEADER_SIZE + sender_payload_size );
            printf( "%s %08x %d %s %s\n", "sent:", session_id, seq_no, "DATA",
                    payload );
            fflush( stdout );
            sendto( sockfd, buffer, HEADER_SIZE + sender_payload_size, 0,
                    servaddr, socksize );
            TIMER = true;
            time( &START_TIME );
            STATE = READY_TIMER;
