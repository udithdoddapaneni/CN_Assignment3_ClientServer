...
unordered_map<pthread_t, queue<sendargs>> freelist; // memory blocks that need to be freed on exit
unordered_map<uint32_t, session_state> session_map; // session : pthread_t, seq_no
...
void handle_thread_exit(int SIGNAL){
    pthread_exit(NULL);
}
...
void* session_listen(void *){
    ...
    while (continue_session){
        ...
        if (!freelist[session_thread].empty()){
            sendargs args = freelist[session_thread].front(); freelist[session_thread].pop();
            sendflag = true;
            temp = args;
            ...
            if (command == GOODBYE){
                ...
                printf("0x%08x [%d] %s\n", session_id, seq_no, "Session started");
            }
            time(&current_time);
    ...
    pthread_mutex_lock(&lock);
        if (thread_map.find(session_thread) != thread_map.end()){
            ...
            if (session_map.find(session_id) != session_map.end()) session_map.erase(session_id);
            while (!freelist[session_thread].empty()){
                auto front = freelist[session_thread].front(); freelist[session_thread].pop();
            }
...
void Response(sendargs &args){
    ...
    if (MessageFactory::getMagic(input_buffer) != MAGIC || MessageFactory::getVersion(input_buffer) != VERSION){
        // silently discard the packet as it doesn't match the given requirements
        return;
    }
    ...
    if (command == HELLO){
        ...
        pthread_mutex_lock(&lock);
            if (session_map.find(session_id) == session_map.end()){
                ...
                printf("0x%08x [%d] %s\n", session_id, sequence_no, "duplicate packet");
            }
            else if (session_map[session_id].seq_no + 1 < sequence_no){
                for (auto i = session_map[session_id].seq_no; i < sequence_no; i++)
                ...
                MessageFactory::setCommand(GOODBYE, args.buffer);
                append_session(session_id, args);
                session_map.erase(session_id); // no more messages to this session again
            }
            else{
...
int main(){
    ...
    getnameinfo(
        servinfo->ai_addr, (socklen_t) servinfo->ai_addrlen, host, NI_MAXHOST, port, NI_MAXSERV, 0
    );
