...
uint64_t htonll(uint64_t hostbyte){
    ...
    if (__BYTE_ORDER == __LITTLE_ENDIAN){
        return (((uint64_t)htonl(hostbyte & 0xFFFFFFFFULL)) << 32) | htonl(hostbyte >> 32);
    }
    else{
        return hostbyte;
    }
...
uint64_t ntohll(uint64_t netbyte){
    if (__BYTE_ORDER == __LITTLE_ENDIAN){
        return (((uint64_t)ntohl(netbyte & 0xFFFFFFFFULL)) << 32) | ntohl(netbyte >> 32);
    }
    else{
        return netbyte;
    }
...
class MessageFactory{
    ...
    static void WriteMessage(char *header, char *payload, char *output_buffer, size_t buffer_size){
        memcpy(output_buffer, header, HEADER_SIZE);
        memcpy(output_buffer+HEADER_SIZE, payload, buffer_size-HEADER_SIZE);
    }
    static void ReadHeader(char *buffer, char* header){
        memcpy(header, buffer, HEADER_SIZE);
    }
    static void ReadPayload(char *input_buffer, char *output_buffer, size_t n){
